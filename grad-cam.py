...
i=0##testing in what
resnet = models.resnet101(pretrained=True)#这里单独加载一个包含全连接层的resnet50模型
image = []
...
def show_cam_on_image(img, mask,name):
	...
	cam = cam / np.max(cam)
	cv2.imwrite("heatmap/cam/cam_{}.jpg".format(name), np.uint8(255 * cam))
class GradCam:
...
def get_args():
	...
	parser.add_argument('--image-path', type=str, default='heatmap/examples/',
...
if __name__ == '__main__':
	...
	del model.fc
	print(model)
	print('====================={} structure printed========================='.format(PREDICT_MODEL_NAME))
	...
	for root,dirs,filenamelist in x:
	#print(type(grad_cam))
	...
	for img in image:
		...
		print('{}   input-size={}'.format(filenames[i],input.size()))
	...
	# Otherwise, targets the requested index.
		...
		show_cam_on_image(img, mask,filenames[i])


		gb_model = GuidedBackpropReLUModel(model = models.resnet50(pretrained=True), use_cuda=args.use_cuda)
		gb = gb_model(input, index=target_index)
		if not os.path.exists('heatmap/gb'):
			os.mkdir('heatmap/gb')
		if not os.path.exists('heatmap/camgb'):
			os.mkdir('heatmap/camgb')
		utils.save_image(torch.from_numpy(gb), './heatmap/gb/gb_{}.jpg'.format(filenames[i]))
		cam_mask = np.zeros(gb.shape)
		for j in range(0, gb.shape[0]):
			cam_mask[j, :, :] = mask
		cam_gb = np.multiply(cam_mask, gb)
		utils.save_image(torch.from_numpy(cam_gb), 'heatmap/camgb/cam_gb_{}.jpg'.format(filenames[i]))
